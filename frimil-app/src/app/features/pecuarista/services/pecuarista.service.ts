...
export class PecuaristaService {
  ...
  getPecuaristas(): Observable<Pecuarista[]> {
    ...
    return this.http.get<ApiResponse<Pecuarista[]>>(this.apiUrl).pipe(
      retry(2),
  ...
  getPecuarista(id: number): Observable<Pecuarista> {
    ...
    return this.http.get<ApiResponse<Pecuarista>>(`${this.apiUrl}/${id}`).pipe(
      map((response) => response.data!),
  ...
  createPecuarista(pecuarista: PecuaristaForm): Observable<Pecuarista> {
    ...
    return this.http
      .post<ApiResponse<Pecuarista>>(this.apiUrl, pecuarista)
      .pipe(
        retry(1),
        map((response) => response.data!),
        tap((newPecuarista) => this.appState.addPecuarista(newPecuarista)),
  ...
  updatePecuarista(
    id: number,
    pecuarista: PecuaristaForm
  ): Observable<Pecuarista> {
    if (this.useMockData) {
      return this.mockService.updatePecuarista(id, pecuarista);
    }
    return this.http
      .put<ApiResponse<Pecuarista>>(`${this.apiUrl}/${id}`, pecuarista)
      .pipe(
        map((response) => response.data!),
        catchError(this.handleError)
      );
  ...
  getFazendas(): Observable<string[]> {
    ...
    return this.http.get<ApiResponse<string[]>>(`${this.apiUrl}/fazendas`).pipe(
      map((response) => response.data || []),
      catchError(this.handleError)
    );
  }
  ...
  exportToExcel(): Observable<Blob> {
    ...
    return this.http
      .get(`${this.apiUrl}/export`, { responseType: 'blob' })
  ...
  private handleError(error: any): Observable<never> {
    ...
    return throwError(
      () =>
        new Error(error.error?.message || 'Erro na comunicação com o servidor')
    );
  }
}
